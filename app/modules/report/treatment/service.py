from datetime import datetime
from app.common.errors import AppError
from .repository import TreatmentReportRepository


class TreatmentReportService:

    @staticmethod
    def _norm_str(v) -> str:
        return (v or "").strip()

    @staticmethod
    def _validate_date(date_str: str, field_name: str) -> str:
        value = TreatmentReportService._norm_str(date_str)

        if not value:
            raise AppError(f"{field_name} wajib diisi.", 400)

        try:
            datetime.strptime(value, "%Y-%m-%d")
        except ValueError:
            raise AppError(f"{field_name} harus berformat YYYY-MM-DD.", 400)

        return value

    @staticmethod
    def _validate_period(start_date: str, end_date: str):
        s = TreatmentReportService._validate_date(start_date, "start_date")
        e = TreatmentReportService._validate_date(end_date, "end_date")

        if s > e:
            raise AppError("start_date tidak boleh lebih besar dari end_date.", 400)

        return s, e


    @staticmethod
    def get_summary(start_date: str, end_date: str):

        s, e = TreatmentReportService._validate_period(start_date, end_date)

        rows = TreatmentReportRepository.get_summary(
            start_date=s,
            end_date=e
        )

        return {
            "start_date": s,
            "end_date": e,
            "rows": rows
        }


    @staticmethod
    def get_detail(start_date: str, end_date: str):

        s, e = TreatmentReportService._validate_period(start_date, end_date)

        rows = TreatmentReportRepository.get_detail(
            start_date=s,
            end_date=e
        )

        return {
            "start_date": s,
            "end_date": e,
            "rows": rows
        }